...
$(document).ready(function(){
    ...
    $('#mainContent').on('click','.jsplumb-close-icon',function(){        
        ...
        var objType = $(this).closest('.plumbTable').data('objectType');
        var objName = $(this).data(objType+'Name');
        console.log('type',objType);
        console.log('name',objName);
        if(c){                                                                            
            if(objType == 'table'){
                console.log('it has dropped?',$('.tbl-treeview li[data-table-name="'+objName+'"]').data('HasDropped'));
                $('.tbl-treeview li[data-table-name="'+objName+'"]').data('HasDropped',false);
            }else{
                $('.view-treeview li[data-view-name="'+objName+'"]').data('HasDropped',false);
            }
    ...
    jsPlumb.ready(function(){
        ...
        drop: function(event,ui){
            ...
            var tipe = (droppedObject.data('tableName')!= null || typeof(droppedObject.data('tableName'))!= 'undefined')
                ? 'table':'view';
            console.log('dropped object--data',$(droppedObject).data('hasDropped'));
            if (!$(droppedObject).data('hasDropped')){
                $.ajax({
            ...
            }else alert('the object already in the context');            
        }
    })
