...
$(document).ready(function(){
	...
	$('.tbl-treeview li, .view-treeview li').draggable({
	 ...
	 var newNode = function(resultId,type,resultColumns,x, y) {
        ...
        headerDiv.innerHTML='<i class="fa fa-'+iconHeader+'"></i>&nbsp;' + resultId +'<span class="jsplumb-close-icon"><i class="fa fa-times"></i></span>';
    ...
    $('div.plumbTable').on('click','.jsplumb-close-icon',function(){        
        var c = confirm('really want to close this object?');
        if(c){
            alert('deleted ...');
            setTimeout(function(){
                alert('just kidding :p');
            },900)
        }
    })
})